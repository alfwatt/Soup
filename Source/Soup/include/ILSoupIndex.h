...
/// many methods will return the same emptyCursor, but checking for object identity is not recommended
...
/// @param entryIndex — index of the entry we want to access
...
/// @protocol maintains an index of items by an indexPath provided
/// <a id="ILSoupIndex"></a>
...
/// an ancestry index mantains the chain of ancestery for an entry
/// the ancestry index is an identity index with the entry key pointing to the ancestor
///
/// A given entry MAY have one Ancestor and multiple Descendants
/// (i.e. an entry can be mutated multiple times, creating multiple lines of descendants)
///
...
- (id<ILSoupCursor>) ancestryOf:(id<ILSoupEntry>) descendant;

/// every entry can have multiple immediate descendants
/// @returns a cursor with the immediate descendants of this entry
- (id<ILSoupCursor>) descendantsOf:(id<ILSoupEntry>) ancestor;

/// @returns all entries in the index which do not have an ancestor, i.e. the roots of the ancestry graph
...
/// only indexes values which are numbers
